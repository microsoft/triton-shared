...
// CHECK:           [[VAR_2_:%.+]] = tensor.empty() : tensor<128x!tt.ptr<bf16, 1>>
// CHECK:           [[VAR_3_:%.+]] = linalg.fill ins([[PARAM_0_]] : !tt.ptr<bf16, 1>) outs([[VAR_2_]] : tensor<128x!tt.ptr<bf16, 1>>) -> tensor<128x!tt.ptr<bf16, 1>>
// CHECK:           [[VAR_4_:%.+]] = linalg.generic {indexing_maps = [#map, #map, #map], iterator_types = ["parallel"]} ins([[VAR_3_]], [[VAR_1_]] : tensor<128x!tt.ptr<bf16, 1>>, tensor<128xi32>) outs([[VAR_3_]] : tensor<128x!tt.ptr<bf16, 1>>) {
// CHECK:           ^bb0([[in_:%.*]]: !tt.ptr<bf16, 1>, [[in_0_:%.*]]: i32, [[out_:%.*]]: !tt.ptr<bf16, 1>):
...
// CHECK:           tt.store [[PARAM_1_]], [[VAR_7_]] {cache = 1 : i32, evict = 1 : i32} : bf16
// CHECK:           return
// CHECK:         }
