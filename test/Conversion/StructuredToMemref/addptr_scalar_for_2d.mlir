...
// CHECK:           [[VAR_reinterpret_cast_0_:%.+]] = memref.reinterpret_cast [[base_buffer_]] to offset: {{.}}[[VAR_4_]]{{.}}, sizes: [1], strides: [1] : memref<f32> to memref<1xf32, strided<[1], offset: ?>>
// CHECK-DAG:       [[VAR_5_:%.+]]:3 = scf.for [[VAR_arg11_:%.+]] = [[CST_0_]] to [[CST_12_]] step [[CST_3_]] iter_args([[VAR_arg12_:%.+]] = [[VAR_1_]], [[VAR_arg13_:%.+]] = [[VAR_reinterpret_cast_0_]], [[VAR_arg14_:%.+]] = [[VAR_3_]]) -> (tensor<128x128xf32>, memref<1xf32, strided<[1], offset: ?>>, index) {
// CHECK-DAG:         [[VAR_9_:%.+]] = arith.addi [[VAR_arg14_]], [[CST_128_]] : index
// CHECK-NOT: separator of consecutive DAGs
// CHECK-DAG:         [[VAR_reinterpret_cast_2_:%.+]] = memref.reinterpret_cast [[PARAM_1_]] to offset: {{.}}[[VAR_9_]]{{.}}, sizes: [128, 128], strides: [1, 1] : memref<*xf32> to memref<128x128xf32, strided<[1, 1], offset: ?>>
...
// CHECK:             [[VAR_13_:%.+]] = arith.addi [[offset_4_]], [[VAR_arg11_]] : index
// CHECK-DAG:         [[VAR_reinterpret_cast_7_:%.+]] = memref.reinterpret_cast [[base_buffer_3_]] to offset: {{.}}[[VAR_13_]]{{.}}, sizes: [1], strides: [1] : memref<f32> to memref<1xf32, strided<[1], offset: ?>>
// CHECK-DAG:         [[VAR_14_:%.+]] = arith.addi [[VAR_arg14_]], [[VAR_arg11_]] : index
// CHECK:             scf.yield [[VAR_12_]], [[VAR_reinterpret_cast_7_]], [[VAR_14_]] : tensor<128x128xf32>, memref<1xf32, strided<[1], offset: ?>>, index
// CHECK:           }
...
// CHECK:           [[VAR_reinterpret_cast_1_:%.+]] = memref.reinterpret_cast [[PARAM_0_]] to offset: {{.}}[[VAR_8_]]{{.}}, sizes: [128, 128], strides: [1, 1] : memref<*xf32> to memref<128x128xf32, strided<[1, 1], offset: ?>>
// CHECK:           bufferization.materialize_in_destination [[VAR_5_]]#0 in writable [[VAR_reinterpret_cast_1_]] : (tensor<128x128xf32>, memref<128x128xf32, strided<[1, 1], offset: ?>>) -> ()
// CHECK:           return
// CHECK:         }
