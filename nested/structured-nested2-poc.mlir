module {
  tt.func public @nested1(%arg0: !tt.ptr<f32>, %arg1: !tt.ptr<f32>, %arg2: i32, %arg3: i32, %arg4: i32, %arg5: i32, %arg6: i32, %arg7: i32) attributes {noinline = false} {
    ...
    %6:2 = scf.for %arg8 = %c0_i32 to %c2_i32 step %c1_i32 iter_args(%arg9 = %c0, %arg10 = %c0) -> (index, index)  : i32 {
      ...
      %loop:2 = scf.for %i = %c0_i32 to %c2_i32 step %c1_i32 iter_args(%index0 = %10, %index1 = %11) -> (index, index)  : i32 {
        %ptr0 = tts.make_tptr %arg1 to sizes: [2, 2], strides: [%2, %3], offsets: [%index1, %c0], shape: [0, 0], order: [] : <f32> to tensor<2x2x!tt.ptr<f32>>
        %ptr1 = tts.make_tptr %arg0 to sizes: [2, 2], strides: [%0, %1], offsets: [%index0, %c0], shape: [0, 0], order: [] : <f32> to tensor<2x2x!tt.ptr<f32>>
        %t = "tts.load"(%ptr1) <{operandSegmentSizes = array<i32: 1, 0, 0>, static_mask_dims = array<i64>}> : (tensor<2x2x!tt.ptr<f32>>) -> tensor<2x2xf32>
        "tts.store"(%ptr0, %t) <{static_mask_dims = array<i64>}> : (tensor<2x2x!tt.ptr<f32>>, tensor<2x2xf32>) -> ()
        %new_index_0 = arith.addi %index0, %5 : index
        %new_index_1 = arith.addi %index1, %5 : index
        scf.yield %new_index_0, %new_index_1 : index, index
      }
      scf.yield %loop#0, %loop#1 : index, index
